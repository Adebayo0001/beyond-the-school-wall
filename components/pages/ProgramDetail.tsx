      ...
      <div className="flex border-b border-[#e8e5e0] mb-4 gap-1 overflow-x-auto pb-1 scrollbar-none">
        {models.map((m, idx) => (
          <button
            ...
            className={`px-3.5 py-1.5 text-xs rounded-lg font-bold whitespace-nowrap transition-all ${activeModel === idx ? 'bg-[#F16736] text-white shadow-sm' : 'hover:bg-[#fff1eb] hover:text-[#F16736] text-[#6b6b6b]'}`}
          >
            Model {idx + 1}
          </button>
        ))}
...
const ProgramDetail = ({ overrideSlug, slug: propSlug }: { overrideSlug?: string; slug?: string }) => {
  const params = useParams();
  const urlSlug = (params?.slug as string) || '';
  const slug = overrideSlug || propSlug || urlSlug;
  ...
  return (
    <div className="bg-white min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            <Breadcrumbs 
              items={[
                { label: 'Home', href: '/' },
                { label: 'Curriculum & Programs', href: '/programs' },
                { label: program.title }
              ]} 
              variant="light" 
            />
            <Link href="/programs" 
              className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-neutral-600 hover:text-[#F16736] transition-colors group bg-white/80 backdrop-blur-sm px-3.5 py-1.5 rounded-full border border-[#e8e5e0]"
            >
              <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-1 transition-transform" /> Back to Curriculum
            </Link>
          </div>
              ...
              <div className="p-8 md:p-12">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-8 border-t border-[#e8e5e0] gap-4">
                  <button 
                    ...
                    className={`px-8 py-4 rounded-lg font-bold transition-all flex items-center justify-center gap-3 w-full sm:w-auto cursor-pointer ${
                      completedModules.includes(activeWeek) 
                        ? 'bg-neutral-100 text-neutral-400 cursor-not-allowed' 
                        : 'bg-[#1e1e1e] hover:bg-[#F16736] text-white hover:shadow-lg hover:-translate-y-0.5'
                    }`}
                  >
                  ...
                  {activeWeek < program.curriculum.length - 1 && (
                    <button 
                      ...
                      className="text-sm font-bold text-[#F16736] hover:underline flex items-center justify-center gap-1 w-full sm:w-auto cursor-pointer"
                    >
      ...
      <section className="py-32 bg-gradient-to-br from-[#F16736] via-[#ea580c] to-[#d94815] relative overflow-hidden text-white">
        ...
        />
        <div className="absolute inset-0 opacity-[0.08]" style={{ backgroundImage: 'radial-gradient(circle, #fff 1px, transparent 1px)', backgroundSize: '40px 40px' }} />

        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-md bg-white/15 border border-white/25 text-white text-xs font-black uppercase tracking-wider mb-8 backdrop-blur-sm shadow-sm">
            <Users size={12} className="text-white" /> {program.cohortDate}
          </div>
          ...
          <p className="text-white/90 text-lg md:text-xl xl:text-2xl mb-12 leading-relaxed font-medium max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <button className="w-full sm:w-auto md:w-auto group relative px-10 py-5 xl:px-12 xl:py-6 bg-white text-[#F16736] text-lg font-black rounded-lg overflow-hidden transition-all hover:shadow-[0_0_50px_rgba(255,255,255,0.4)] active:scale-95 flex items-center justify-center gap-3 hover:scale-105 cursor-pointer">
            ...
            <button className="w-full sm:w-auto md:w-auto px-10 py-5 xl:px-12 xl:py-6 bg-black/20 hover:bg-black/30 border border-white/30 text-white font-bold rounded-lg transition-all flex items-center justify-center gap-3 backdrop-blur-sm cursor-pointer">
              <MessageCircle size={20} className="text-white" /> Inquire via Telegram
            </button>
          </div>
        </div>
      </section>
