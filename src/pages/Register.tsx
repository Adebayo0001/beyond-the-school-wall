...
const Register = ({ isLuminaire = false }: RegisterProps) => {
  ...
  const handleRegister = (e: FormEvent) => {
    ...
    const newUser: User = {
      ...
      enrolledSchools: isLuminaire ? [] : ['the-skill-hut'], // default auto enrol
      completedLessons: [],
      playgroundScores: [],
      streak: 1,
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-16">
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block text-xs font-black uppercase tracking-wider text-neutral-600 mb-2">Full Name</label>
              <div className="relative">
                <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
                <input
                  type="text"
                  required
                  placeholder="e.g. Abidemi Bello"
                  value={name}
            ...
            <div>
              <label className="block text-xs font-black uppercase tracking-wider text-neutral-600 mb-2">Choose passcode</label>
              <div className="relative">
          ...
          <div className="text-center text-xs font-semibold text-neutral-500 pt-2">
            ...
            <Link to={isLuminaire ? `/luminaire/login` : '/login'} className="text-[#F16736] font-extrabold hover:underline">
              Log in instead
            </Link>
          </div>
        </div>
